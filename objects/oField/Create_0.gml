...
function HighlightMoveableSpaces(piece)
{
	spacesToHighlight = piece.GetMoveableSpaces();
	
	for(var i=0; i<array_length(spacesToHighlight); i++)
	{spacesToHighlight[i].BecomeSelectable();}
	
	tilesSelectable=true;
	rowSelected = piece.row;
	colSelected = piece.column;
	pieceSelected = piece;
}
